	...
	void morphologyEx(const Mat& src, Mat& dst, Mor EDOC, int kernel) {
		...
		case ERODE:
			...
			while (i--) {
				...
				while (j--) {
					if (src.get(i, j).R) {
						for (i_mask = i - halfKernelSize; i_mask <= i + halfKernelSize; ++i_mask) {
							for (j_mask = j - halfKernelSize; j_mask <= j + halfKernelSize; ++j_mask) {
								if (!(i == i_mask && j == j_mask) && i_mask >= 0 && i_mask < src.rows && j_mask >= 0 && j_mask < src.cols && !src.get(i_mask, j_mask).R)
									dst.set(i, j, 2);
							}
						}
					}
			...
			while (i--) {
				...
				while (j--) {
					if (!src.get(i, j).R) {
						for (i_mask = i - halfKernelSize; i_mask <= i + halfKernelSize; ++i_mask) {
							for (j_mask = j - halfKernelSize; j_mask <= j + halfKernelSize; ++j_mask) {
								if (!(i == i_mask && j == j_mask) && i_mask >= 0 && i_mask < src.rows && j_mask >= 0 && j_mask < src.cols && src.get(i_mask, j_mask).R == 1)
									dst.set(i, j, 2);
							}
						}
					}
		...
		case OPEN:
			...
			morphologyEx(src, dst, DILATE, kernel);
			break;
		case CLOSE:
			morphologyEx(src, dst, DILATE, kernel);
			morphologyEx(src, dst, ERODE, kernel);
